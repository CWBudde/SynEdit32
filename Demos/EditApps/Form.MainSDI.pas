...
uses
  ...
  Menus, ActnList, Actions, ComCtrls, UnitEditAppIntfs, Form.Main;
...
type
  ...
  private
    FEditorForm: IEditor;
  protected
    function DoCreateEditor(AFileName: TFileName): IEditor; override;
  end;
...
function TSDIMainForm.DoCreateEditor(AFileName: TFileName): IEditor;
begin
  if FEditorForm <> nil then
    Result := FEditorForm
  else if GEditorFactory <> nil then begin
    FEditorForm := GEditorFactory.CreateBorderless(Self);
    Result := FEditorForm;
  end else
    Result := nil;
end;
