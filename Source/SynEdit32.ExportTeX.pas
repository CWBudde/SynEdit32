...
type
  TSynEdit32ExporterTeX = class(TSynEdit32CustomExporter)
  private
    FMargin: Integer;
    FLastAttri: TSynEdit32HighlighterAttributes;
    function AttriToCommand(Attri: TSynEdit32HighlighterAttributes;
      UniqueAttriName: string): string;
    function AttriToCommandCallback(Highlighter: TSynEdit32CustomHighlighter;
      Attri: TSynEdit32HighlighterAttributes; UniqueAttriName: string;
      Params: array of Pointer): Boolean;
    function CommandNameCallback(Highlighter: TSynEdit32CustomHighlighter;
      Attri: TSynEdit32HighlighterAttributes; UniqueAttriName: string;
      Params: array of Pointer): Boolean;
    function GetCommandName(Highlighter: TSynEdit32CustomHighlighter;
      Attri: TSynEdit32HighlighterAttributes): string;
  ...
  protected
    ...
    procedure SetTokenAttribute(Attri: TSynEdit32HighlighterAttributes); override;
...
{ TSynEdit32ExporterTeX }

constructor TSynEdit32ExporterTeX.Create(AOwner: TComponent);
begin
...
function TSynEdit32ExporterTeX.AttriToCommandCallback(
  Highlighter: TSynEdit32CustomHighlighter; Attri: TSynEdit32HighlighterAttributes;
...
function TSynEdit32ExporterTeX.AttriToCommand(Attri: TSynEdit32HighlighterAttributes;
  UniqueAttriName: string): string;
const
  NewCommand    = '\newcommand{\%s}[1]{%s#1%s}';
  SBold         = '\textbf{';
...
function TSynEdit32ExporterTeX.CommandNameCallback(
  Highlighter: TSynEdit32CustomHighlighter; Attri: TSynEdit32HighlighterAttributes;
  UniqueAttriName: string; Params: array of Pointer): Boolean;
var
  AttriToFind: TSynEdit32HighlighterAttributes;
  CommandName: ^string;
begin
...
procedure TSynEdit32ExporterTeX.FormatToken(Token: UnicodeString);
var
  CommandName: string;
begin
...
procedure TSynEdit32ExporterTeX.FormatNewLine;
begin
  AddData('\\' + SLineBreak);
end;

// do nothing with these
procedure TSynEdit32ExporterTeX.FormatAfterLastAttribute;
begin
end;

procedure TSynEdit32ExporterTeX.FormatAttributeDone;
begin
end;

procedure TSynEdit32ExporterTeX.FormatAttributeInit;
begin
end;

procedure TSynEdit32ExporterTeX.FormatBeforeFirstAttribute;
begin
end;

function TSynEdit32ExporterTeX.GetCommandName(Highlighter: TSynEdit32CustomHighlighter;
  Attri: TSynEdit32HighlighterAttributes): string;
begin
  EnumHighlighterAttris(Highlighter, False, CommandNameCallback, [Attri, @Result]);
end;

function TSynEdit32ExporterTeX.GetFooter: UnicodeString;
begin
...
function TSynEdit32ExporterTeX.GetFormatName: string;
begin
  Result := SYNS_ExporterFormatTeX;
end;

function TSynEdit32ExporterTeX.GetHeader: UnicodeString;
const
...
function TSynEdit32ExporterTeX.GetNewCommands: string;
const
...
function TSynEdit32ExporterTeX.MakeValidName(Name: string): string;
var
  i: Integer;
begin
  Result := Name;
...
function TSynEdit32ExporterTeX.ReplaceReservedChar(AChar: WideChar): UnicodeString;
begin
  case AChar of
    '{': Result := '\{';
    '}': Result := '\}';
...
procedure TSynEdit32ExporterTeX.SetTokenAttribute(Attri: TSynEdit32HighlighterAttributes);
begin
  FLastAttri := Attri;
end;

function TSynEdit32ExporterTeX.SupportedEncodings: TSynEncodings;
begin
  Result := [seAnsi];
end;

function TSynEdit32ExporterTeX.UseBom: Boolean;
begin
  Result := False;
end;
