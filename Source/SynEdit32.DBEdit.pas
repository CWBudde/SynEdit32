...
uses
  Windows, Messages, Controls, DbCtrls, SysUtils, Classes, DB,
  SynEdit32, SynEdit32.KeyCmds;

type
  TCustomDBSynEdit32 = class(TCustomSynEdit32)
  private
...
constructor TCustomDBSynEdit32.Create(AOwner: TComponent);
begin
...
destructor TCustomDBSynEdit32.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
  inherited Destroy;
...
{$IFNDEF SYN_CLX}
procedure TCustomDBSynEdit32.CMEnter(var Msg: TCMEnter);
begin
  SetEditing(True);
  inherited;
end;

procedure TCustomDBSynEdit32.CMExit(var Msg: TCMExit);
begin
  try
...
procedure TCustomDBSynEdit32.CMGetDataLink(var Msg: TMessage);
begin
  Msg.Result := LRESULT(FDataLink);
end;
{$ENDIF}

procedure TCustomDBSynEdit32.DataChange(Sender: TObject);
begin
...
procedure TCustomDBSynEdit32.DragDrop(Source: TObject; X, Y: Integer);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TCustomDBSynEdit32.EditingChange(Sender: TObject);
begin
...
procedure TCustomDBSynEdit32.ExecuteCommand(Command: TSynEditorCommand;
...
function TCustomDBSynEdit32.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

function TCustomDBSynEdit32.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomDBSynEdit32.GetField: TField;
begin
  Result := FDataLink.Field;
end;

function TCustomDBSynEdit32.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TCustomDBSynEdit32.Loaded;
begin
  inherited Loaded;
  if csDesigning in ComponentState then
    DataChange(Self);
...
procedure TCustomDBSynEdit32.LoadMemo;
{$IFDEF SYN_COMPILER_3_UP}
var
  BlobStream: TStream;
{$ELSE}
...
procedure TCustomDBSynEdit32.DoChange;
begin
  FDataLink.Modified;
  inherited;
end;

procedure TCustomDBSynEdit32.Notification(AComponent: TComponent;
...
procedure TCustomDBSynEdit32.SetDataField(const Value: string);
begin
  FDataLink.FieldName := Value;
end;

procedure TCustomDBSynEdit32.SetDataSource(Value: TDataSource);
begin
...
procedure TCustomDBSynEdit32.SetEditing(Value: Boolean);
begin
...
procedure TCustomDBSynEdit32.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

procedure TCustomDBSynEdit32.UpdateData(Sender: TObject);
{$IFDEF SYN_COMPILER_3_UP}
var
  BlobStream: TStream;
{$ENDIF}
