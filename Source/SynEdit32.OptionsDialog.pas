...
{$ENDIF}
  ...
  private
    ...
    procedure FillInKeystrokeInfo(AKey: TSynEdit32Keystroke; AItem: TListItem);
  public
    eKeyShort2: TSynEdit32HotKey;
    eKeyShort1: TSynEdit32HotKey;
    {$IFNDEF SYN_DELPHI_4_UP}
  ...
  private
    ...
    FFont: TFont;
    FBookmarks: TSynEdit32BookMarkOpt;
  ...
  published
    ...
    property Gutter : TSynEdit32Gutter read FSynGutter write SetSynGutter;
    ...
    property InsertCaret : TSynEdit32CaretType read FInsertCaret write FInsertCaret;
    property OverwriteCaret : TSynEdit32CaretType read FOverwriteCaret write FOverwriteCaret;
    ...
    property Keystrokes : TSynEdit32KeyStrokes read FKeystrokes write SetKeystrokes;
  end;
...
begin
  if Assigned(Source) and (Source is TCustomSynEdit32) then
  begin
    Self.Font.Assign(TCustomSynEdit32(Source).Font);
    Self.BookmarkOptions.Assign(TCustomSynEdit32(Source).BookmarkOptions);
    Self.Gutter.Assign(TCustomSynEdit32(Source).Gutter);
    Self.Keystrokes.Assign(TCustomSynEdit32(Source).Keystrokes);
    Self.SelectedColor.Assign(TCustomSynEdit32(Source).SelectedColor);

    Self.Color := TCustomSynEdit32(Source).Color;
    Self.Options := TCustomSynEdit32(Source).Options;
    Self.ExtraLineSpacing := TCustomSynEdit32(Source).ExtraLineSpacing;
    Self.HideSelection := TCustomSynEdit32(Source).HideSelection;
    Self.InsertCaret := TCustomSynEdit32(Source).InsertCaret;
    Self.OverwriteCaret := TCustomSynEdit32(Source).OverwriteCaret;
    Self.MaxScrollWidth := TCustomSynEdit32(Source).MaxScrollWidth;
    Self.MaxUndo := TCustomSynEdit32(Source).MaxUndo;
    Self.RightEdge := TCustomSynEdit32(Source).RightEdge;
    Self.RightEdgeColor := TCustomSynEdit32(Source).RightEdgeColor;
    Self.TabWidth := TCustomSynEdit32(Source).TabWidth;
    Self.WantTabs := TCustomSynEdit32(Source).WantTabs;
  end else
    inherited;
end;
...
begin
  if Assigned(Dest) and (Dest is TCustomSynEdit32) then
  begin
    TCustomSynEdit32(Dest).Font.Assign(Self.Font);
    TCustomSynEdit32(Dest).BookmarkOptions.Assign(Self.BookmarkOptions);
    TCustomSynEdit32(Dest).Gutter.Assign(Self.Gutter);
    TCustomSynEdit32(Dest).Keystrokes.Assign(Self.Keystrokes);
    TCustomSynEdit32(Dest).SelectedColor.Assign(Self.SelectedColor);

    TCustomSynEdit32(Dest).Color := Self.Color;
    TCustomSynEdit32(Dest).Options := Self.Options;
    TCustomSynEdit32(Dest).ExtraLineSpacing := Self.ExtraLineSpacing;
    TCustomSynEdit32(Dest).HideSelection := Self.HideSelection;
    TCustomSynEdit32(Dest).InsertCaret := Self.InsertCaret;
    TCustomSynEdit32(Dest).OverwriteCaret := Self.OverwriteCaret;
    TCustomSynEdit32(Dest).MaxScrollWidth := Self.MaxScrollWidth;
    TCustomSynEdit32(Dest).MaxUndo := Self.MaxUndo;
    TCustomSynEdit32(Dest).RightEdge := Self.RightEdge;
    TCustomSynEdit32(Dest).RightEdgeColor := Self.RightEdgeColor;
    TCustomSynEdit32(Dest).TabWidth := Self.TabWidth;
    TCustomSynEdit32(Dest).WantTabs := Self.WantTabs;
  end else
    inherited;
end;
...
begin
  inherited;
  FBookmarks := TSynEdit32BookMarkOpt.Create(Self);
  FKeystrokes := TSynEdit32KeyStrokes.Create(Self);
  FSynGutter := TSynEdit32Gutter.Create;
...
procedure TSynEditorOptionsContainer.SetBookMarks(
  const Value: TSynEdit32BookMarkOpt);
begin
...
procedure TSynEditorOptionsContainer.SetKeystrokes(
  const Value: TSynEdit32KeyStrokes);
begin
...
procedure TSynEditorOptionsContainer.SetSynGutter(const Value: TSynEdit32Gutter);
begin
...
begin
  ...
  //Caret
  FSynEdit.InsertCaret := TSynEdit32CaretType(cInsertCaret.ItemIndex);
  FSynEdit.OverwriteCaret := TSynEdit32CaretType(cOverwriteCaret.ItemIndex);
end;
...
begin
  ...
  eKeyShort1 := TSynEdit32HotKey.Create(Self);
  with eKeyShort1 do
  begin
    Parent := gbKeystrokes;
    Left := 120;
  ...
  eKeyShort2 := TSynEdit32HotKey.Create(Self);
  with eKeyShort2 do
  begin
    Parent := gbKeystrokes;
    Left := 120;
...
begin
  ...
  eKeyShort1.HotKey     := TSynEdit32KeyStroke(KeyList.Selected.Data).ShortCut;
  eKeyShort2.HotKey     := TSynEdit32KeyStroke(KeyList.Selected.Data).ShortCut2;
  FOldSelected := Item;
end;
...
begin
  ...
  TSynEdit32KeyStroke(FOldSelected.Data).Command := Cmd;

  if eKeyShort1.HotKey <> 0 then
    TSynEdit32KeyStroke(FOldSelected.Data).ShortCut := eKeyShort1.HotKey;

  if eKeyShort2.HotKey <> 0 then
    TSynEdit32KeyStroke(FOldSelected.Data).ShortCut2 := eKeyShort2.HotKey;

  FillInKeystrokeInfo(TSynEdit32KeyStroke(FOldSelected.Data), KeyList.Selected);
end;
...
begin
  if KeyList.Selected = nil then Exit;
  TSynEdit32KeyStroke(KeyList.Selected.Data).Free;
...
procedure TfmEditorOptionsDialog.FillInKeystrokeInfo(
  AKey: TSynEdit32KeyStroke; AItem: TListItem);
...
//make sure that it's saved.
  ...
  begin
    if (Item = FOldSelected) and
       ...
       (TSynEdit32KeyStroke(Item.Data).ShortCut <> eKeyShort1.HotKey) or
       (TSynEdit32KeyStroke(Item.Data).ShortCut2 <> eKeyShort2.HotKey)) then
    begin
