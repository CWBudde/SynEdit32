...
procedure TSynEdit32Memo.EMGetSel(var Message: TMessage);
var
  s, e: integer;
begin
  s := GetSelStart;
...
procedure TSynEdit32Memo.EMSetSel(var Message: TMessage);
begin
  SetSelStart(Message.wParam);
  SetSelEnd(Message.lParam);
end;

procedure TSynEdit32Memo.EMSetModify(var Message: TMessage);
begin
  Modified := Message.wParam <> 0;
end;

procedure TSynEdit32Memo.EMGetModify(var Message: TMessage);
begin
  Message.Result := Integer(Modified);
end;

procedure TSynEdit32Memo.EMGetLineCount(var Message: TMessage);
begin
...
procedure TSynEdit32Memo.EMGetSelText(var Message: TMessage);
begin
...
// no terminating #0
procedure TSynEdit32Memo.EMGetLine(var Message: TMessage);
var
...
//(WPARAM) wParam,    // not used; must be zero
//(LPARAM) lParam     // not used; must be zero
procedure TSynEdit32Memo.EMCanUndo(var Message: TMessage);
begin
...
//(WPARAM) wParam,    // not used; must be zero
//(LPARAM) lParam     // not used; must be zero
procedure TSynEdit32Memo.EMUndo(var Message: TMessage);
begin
  Message.Result := Integer(CanUndo);
  Undo;
end;
...
//(WPARAM) wParam,          // not used; must be zero
//(LPARAM) lParam           // not used; must be zero
procedure TSynEdit32Memo.EMGetFirstVisibleLine(var Message: TMessage);
begin
