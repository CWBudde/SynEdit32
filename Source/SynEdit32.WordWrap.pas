...
type
  ...
  TSynEdit32WordWrapPlugin = class(TInterfacedObject, ISynEdit32BufferPlugin)
  private
  ...
  protected
    ...
    property Editor: TCustomSynEdit32 read FEditor;
  public
...
uses
  RTLConsts, Consts, Math, SynEdit32.Unicode;
...
constructor TSynEdit32WordWrapPlugin.Create(aOwner: TCustomSynEdit32);
begin
...
destructor TSynEdit32WordWrapPlugin.Destroy;
begin
  inherited;
...
procedure TSynEdit32WordWrapPlugin.DisplayChanged;
begin
...
function TSynEdit32WordWrapPlugin.GetRowLength(aRow: Integer): Integer;
// aRow is 1-based...
begin
...
procedure TSynEdit32WordWrapPlugin.GrowLines(aMinSize: Integer);
const
  vStepSize = 256;
begin
  Assert(aMinSize > 0);
...
procedure TSynEdit32WordWrapPlugin.GrowRows(aMinSize: Integer);
const
  vStepSize = 512;
begin
  Assert(aMinSize > 0);
...
function TSynEdit32WordWrapPlugin.LinesDeleted(aIndex: Integer; aCount: Integer): Integer;
var
  vStartRow: Integer;
  vEndRow: Integer;
  cLine: Integer;
...
function TSynEdit32WordWrapPlugin.LinesInserted(aIndex: Integer; aCount: Integer): Integer;
var
...
begin
  ...
  if aIndex < LineCount then // no need for MoveLines if inserting at LineCount (TSynEdit32StringList.Add)
  begin
...
function TSynEdit32WordWrapPlugin.LinesPutted(aIndex: Integer; aCount: Integer): Integer;
var
  cLine: Integer;
begin
  if FMaxRowLength = 0 then
...
procedure TSynEdit32WordWrapPlugin.MoveLines(aStart: TLineIndex; aMoveBy: Integer);
var
  vMoveCount: Integer;
begin
  Assert(aMoveBy <> 0);
...
procedure TSynEdit32WordWrapPlugin.MoveRows(aStart: TRowIndex; aMoveBy: Integer);
var
  vMoveCount: Integer;
begin
  Assert(aMoveBy <> 0);
...
procedure TSynEdit32WordWrapPlugin.Reset;
begin
...
function TSynEdit32WordWrapPlugin.ReWrapLine(aIndex: TLineIndex): Integer;
...
begin
  ...
  vLine := TSynEdit32StringList(Editor.Lines).ExpandedStrings[aIndex];
...
procedure TSynEdit32WordWrapPlugin.WrapLines;
var
  cRow: Integer;
  cLine: Integer;
  vLine: UnicodeString;
...
begin
  ...
  begin
    vLine := TSynEdit32StringList(Editor.Lines).ExpandedStrings[cLine];
    vLine := Editor.ExpandAtWideGlyphs(vLine);
...
function TSynEdit32WordWrapPlugin.RowCount: Integer;
begin
  if LineCount > 0 then
...
procedure TSynEdit32WordWrapPlugin.SetEmpty;
begin
  FLineCount := 0;
  // free unsused memory
  TrimArrays;
...
procedure TSynEdit32WordWrapPlugin.TrimArrays;
begin
