...
type
  ...
  TSynEdit32CustomExporter = class(TComponent)
  private
    ...
    procedure SetHighlighter(Value: TSynEdit32CustomHighlighter);
  ...
  protected
    ...
    procedure SetTokenAttribute(Attri: TSynEdit32HighlighterAttributes); virtual;
...
{ TSynEdit32CustomExporter }

constructor TSynEdit32CustomExporter.Create(AOwner: TComponent);
begin
...
destructor TSynEdit32CustomExporter.Destroy;
begin
  FFont.Free;
  FBuffer.Free;
  inherited Destroy;
...
procedure TSynEdit32CustomExporter.AddData(const AText: UnicodeString);
begin
  if AText <> '' then
  begin
    WriteString(AText);
...
procedure TSynEdit32CustomExporter.AddDataNewLine(const AText: UnicodeString);
begin
  AddData(AText);
  AddNewLine;
end;

procedure TSynEdit32CustomExporter.AddNewLine;
begin
  WriteString(WideCRLF);
  FBuffer.SetSize(FBuffer.Position);
end;

procedure TSynEdit32CustomExporter.AssignFont(Value: TFont);
begin
  if Value <> nil then
    FFont.Assign(Value)
  else
...
procedure TSynEdit32CustomExporter.Clear;
begin
  FBuffer.Position := 0;
...
procedure TSynEdit32CustomExporter.CopyToClipboard;
const
...
procedure TSynEdit32CustomExporter.CopyToClipboardFormat(AFormat: UINT);
var
  hData: THandle;
  hDataSize: UINT;
  PtrData: PByte;
...
procedure TSynEdit32CustomExporter.DefineProperties(Filer: TFiler);
begin
  inherited;
{$IFNDEF UNICODE}
...
procedure TSynEdit32CustomExporter.FormatToken(Token: UnicodeString);
begin
  AddData(Token);
end;

function TSynEdit32CustomExporter.GetBufferSize: integer;
begin
  Result := FBuffer.Size;
end;

function TSynEdit32CustomExporter.GetClipboardFormat: UINT;
begin
  Result := FClipboardFormat;
end;

function TSynEdit32CustomExporter.GetFormatName: string;
begin
  Result := '';
end;

procedure TSynEdit32CustomExporter.InsertData(APos: Integer; const AText: UnicodeString);
var
...
function TSynEdit32CustomExporter.ReplaceReservedChars(AToken: UnicodeString): UnicodeString;
var
...
procedure TSynEdit32CustomExporter.SaveToFile(const FileName: UnicodeString);
var
  Stream: TStream;
begin
...
procedure TSynEdit32CustomExporter.SaveToStream(Stream: TStream);
begin
  if UseBOM then
    case Encoding of
      seUTF8:
...
procedure TSynEdit32CustomExporter.SetEncoding(const Value: TSynEncoding);
begin
  ...
  if not (Value in SupportedEncodings) then
    raise ESynEdit32Encoding.CreateFmt(SEncodingError, [EncodingStrs[Value],
      GetFormatName]);
...
procedure TSynEdit32CustomExporter.SetExportAsText(Value: Boolean);
begin
...
procedure TSynEdit32CustomExporter.SetFont(Value: TFont);
begin
  AssignFont(Value);
end;

procedure TSynEdit32CustomExporter.SetHighlighter(Value: TSynEdit32CustomHighlighter);
begin
...
procedure TSynEdit32CustomExporter.SetTitle(const Value: UnicodeString);
begin
...
procedure TSynEdit32CustomExporter.SetTokenAttribute(Attri: TSynEdit32HighlighterAttributes);
var
  ChangedBG: Boolean;
  ChangedFG: Boolean;
...
procedure TSynEdit32CustomExporter.SetUseBackground(const Value: Boolean);
begin
...
function TSynEdit32CustomExporter.StringSize(const AText: UnicodeString): Integer;
begin
  Result := 0;
  case Encoding of
    seUTF8:
...
procedure TSynEdit32CustomExporter.WriteString(const AText: UnicodeString);
var
  UTF8Str: UTF8String;
  AnsiStr: AnsiString;
begin
