...
type
  ...
  private
    ...
    FConditionalCommentAttri: TSynEdit32HighlighterAttributes;
    FConsoleOutputAttri: TSynEdit32HighlighterAttributes;
    FDataTypeAttri: TSynEdit32HighlighterAttributes;
    FDefaultPackageAttri: TSynEdit32HighlighterAttributes;
    FDelimitedIdentifierAttri: TSynEdit32HighlighterAttributes;
    FExceptionAttri: TSynEdit32HighlighterAttributes;
    ...
    FPLSQLAttri: TSynEdit32HighlighterAttributes;
    FSpaceAttri: TSynEdit32HighlighterAttributes;
    FSQLPlusAttri: TSynEdit32HighlighterAttributes;
    FStringAttri: TSynEdit32HighlighterAttributes;
    FSymbolAttri: TSynEdit32HighlighterAttributes;
    FTableNameAttri: TSynEdit32HighlighterAttributes;
    FProcNameAttri: TSynEdit32HighlighterAttributes;
  ...
  published
    property CommentAttri: TSynEdit32HighlighterAttributes read FCommentAttri
      write FCommentAttri;
    property ConditionalCommentAttri: TSynEdit32HighlighterAttributes
      read FConditionalCommentAttri write FConditionalCommentAttri;
    property ConsoleOutputAttri: TSynEdit32HighlighterAttributes
      read FConsoleOutputAttri write FConsoleOutputAttri;
    property DataTypeAttri: TSynEdit32HighlighterAttributes read FDataTypeAttri
      write FDataTypeAttri;
    property DefaultPackageAttri: TSynEdit32HighlighterAttributes
      read FDefaultPackageAttri write FDefaultPackageAttri;
    property DelimitedIdentifierAttri: TSynEdit32HighlighterAttributes
      read FDelimitedIdentifierAttri write FDelimitedIdentifierAttri;
    property ExceptionAttri: TSynEdit32HighlighterAttributes read FExceptionAttri
    ...
    property NumberAttri: TSynEdit32HighlighterAttributes read FNumberAttri
      write FNumberAttri;
    property PLSQLAttri: TSynEdit32HighlighterAttributes read FPLSQLAttri
      write FPLSQLAttri;
    property SpaceAttri: TSynEdit32HighlighterAttributes read FSpaceAttri
      write FSpaceAttri;
    property SQLPlusAttri: TSynEdit32HighlighterAttributes read FSQLPlusAttri
      write FSQLPlusAttri;
    ...
    property SymbolAttri: TSynEdit32HighlighterAttributes read FSymbolAttri
      write FSymbolAttri;
    property ProcNameAttri: TSynEdit32HighlighterAttributes read FProcNameAttri
      write FProcNameAttri;
    property TableNameAttri: TSynEdit32HighlighterAttributes read FTableNameAttri
...
begin
  ...
  FConditionalCommentAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrConditionalComment, SYNS_FriendlyAttrConditionalComment);
  FConditionalCommentAttri.Style := [fsItalic];
  AddAttribute(FConditionalCommentAttri);
  FConsoleOutputAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrConsoleOutput, SYNS_FriendlyAttrConsoleOutput);
  ...
  FDefaultPackageAttri :=
    TSynEdit32HighlighterAttributes.Create(SYNS_AttrDefaultPackage, SYNS_FriendlyAttrDefaultPackage);
  FDefaultPackageAttri.Style := [fsBold];
  AddAttribute(FDefaultPackageAttri);
  FDelimitedIdentifierAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrDelimitedIdentifier, SYNS_FriendlyAttrDelimitedIdentifier);
  AddAttribute(FDelimitedIdentifierAttri);
  FExceptionAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrException, SYNS_FriendlyAttrException);
  ...
  FPLSQLAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrPLSQL, SYNS_FriendlyAttrPLSQL);
  ...
  FSQLPlusAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrSQLPlus, SYNS_FriendlyAttrSQLPlus);
  ...
  FProcNameAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrProcName, SYNS_FriendlyAttrProcName);
  AddAttribute(FProcNameAttri);
  FTableNameAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrTableName, SYNS_FriendlyAttrTableName);
...
begin
  ...
  if FLine[FRun] = #0 then
    ...
    begin
      if (FRun > 0) or (FRange <> rsString) or
        ((FLine[FRun] <> #39) and (FLine[FRun - 1] <> '\')) then
      begin
        FRange := rsString;
        repeat
          if (FLine[FRun] <> '\') and (FLine[FRun + 1] = #39) then
          begin
            Inc(FRun);
            break;
          end;
      ...
      if (FLine[FRun] = #39) and not(FLine[FRun-1] = '\') then
      begin
        Inc(FRun);
        FRange := rsUnknown;
      end;
...
begin
  ...
  begin
    ...
    begin
      FoundDoubleMinus := (FLine[FRun] = '-') and (FLine[FRun + 1] = '-');
      if FoundDoubleMinus then Break;
      Inc(FRun);
    end;
end;
...
begin
  ...
  case FLine[FRun] of
    '*':
      begin
        if (SQLDialect = sqlMySql) and (FLine[FRun + 1] = '!') then
        begin
...
begin
  ...
  if (SQLDialect in [sqlMSSQL7, sqlMSSQL2K]) and (FLine[FRun] = '@') and (FLine[FRun + 1] = '@') then
    IdentProc
  else if (SQLDialect in [sqlMySql, sqlOracle]) and (FLine[FRun] = '@') then
    SymbolProc
  // Oracle uses the ':' character to indicate bind variables
  // Ingres II also uses the ':' character to indicate variables
  else if not (SQLDialect in [sqlOracle, sqlIngres]) and (FLine[FRun] = ':') then
    SymbolProc
  else
  begin
    FTokenID := tkVariable;
