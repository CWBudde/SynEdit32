...
unit SynEdit32.CompletionProposal;
...
uses
  ...
  Types, UITypes,
  {$ENDIF}
  SysUtils, Classes, Windows, Messages, Graphics, Forms, Controls, StdCtrls,
  ExtCtrls, Menus, Dialogs,
  SynEdit32.Types,
  SynEdit32.KeyCmds,
...
type
  ...
  public
    ...
    property CurrentEditor: TCustomSynEdit32 read FCurrentEditor write FCurrentEditor;
  ...
  private
    ...
    function GetPreviousToken(AEditor: TCustomSynEdit32): UnicodeString;
    function GetCurrentInput(AEditor: TCustomSynEdit32): UnicodeString;
    function GetTimerInterval: Integer;
    procedure SetTimerInterval(const Value: Integer);
    function GetEditor(i: Integer): TCustomSynEdit32;
    procedure InternalCancelCompletion; 
  protected
    procedure DoExecute(AEditor: TCustomSynEdit32); virtual;
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
  ...
  public
    ...
    procedure ActivateTimer(ACurrentEditor: TCustomSynEdit32);
    procedure DeactivateTimer;
    property Editors[i: Integer]: TCustomSynEdit32 read GetEditor;
  ...
  protected
    ...
    function GetPreviousToken(Editor: TCustomSynEdit32): UnicodeString;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Execute(Token: UnicodeString; Editor: TCustomSynEdit32);
    procedure ExecuteEx(Token: UnicodeString; Editor: TCustomSynEdit32; LookupIfNotExact: Boolean);
...
uses
  Math, SynEdit32.TextBuffer, SynEdit32.MiscProcs, SynEdit32.KeyConst;
...
begin
  Visible := True;
  if DisplayType = ctCode then
    (CurrentEditor as TCustomSynEdit32).AddFocusControl(Self);
end;

procedure TSynBaseCompletionProposalForm.Deactivate;
begin
  if (DisplayType = ctCode) then
    (CurrentEditor as TCustomSynEdit32).RemoveFocusControl(Self);
  Visible := False;
end;
  ...
  begin
    case Key of
        ...
        begin
          ...
          begin
            ...
            if Assigned(CurrentEditor) then
              (CurrentEditor as TCustomSynEdit32).CommandProcessor(ecLeft, #0, nil);
          end
          else
          begin
      ...
      SYNEDIT_RIGHT:
        begin
          if Assigned(CurrentEditor) then
            with CurrentEditor as TCustomSynEdit32 do
            begin
      ...
      SYNEDIT_BACK:
        ...
        begin
          ...
          begin
            ...
            if Assigned(CurrentEditor) then
              (CurrentEditor as TCustomSynEdit32).CommandProcessor(ecDeleteLastChar, #0, nil);
          end
          else
          begin
            //Since we have control, we need to re-send the key to
      ...
      SYNEDIT_DELETE: if Assigned(CurrentEditor) then
                      (CurrentEditor as TCustomSynEdit32).CommandProcessor(ecDeleteChar, #0, nil);
    end;
  end;
  Invalidate;
end;
...
begin
  ...
  begin
    (CurrentEditor as TCustomSynEdit32).AlwaysShowCaret := FOldShowCaret;
//    (CurrentEditor as TCustomSynEdit32).UpdateCaret;
    if DisplayType = ctCode then
    begin
...
procedure TSynEdit32BaseCompletionProposal.ExecuteEx(s: UnicodeString; x, y: integer; Kind : SynCompletionType);
  ...
  begin
    ...
    begin
      tmpY := tmpY - tmpHeight - (Form.CurrentEditor  as TCustomSynEdit32).LineHeight -2;
      if tmpY < 0 then
        tmpY := 0;
    end;
...
//      ShowWindow(Form.Handle, SW_SHOWNOACTIVATE);
      ...
      {$ELSE}
      Form.Show;
      (Form.CurrentEditor as TCustomSynEdit32).SetFocus;
      {$ENDIF}
      Form.Repaint;
    end;
  end;
...
begin
  ...
  begin
    ...
    if ((F.CurrentEditor as TCustomSynEdit32).Owner is TWinControl) and
       (((F.CurrentEditor as TCustomSynEdit32).Owner as TWinControl).Visible) then
    begin
      ((F.CurrentEditor as TCustomSynEdit32).Owner as TWinControl).SetFocus;
    end;

    (F.CurrentEditor as TCustomSynEdit32).SetFocus;
...
begin
  ...
  if Assigned(F.CurrentEditor) then
    with F.CurrentEditor as TCustomSynEdit32 do
    begin
...
begin
  ...
  begin
    ...
    if (System.Pos(Key, TriggerChars) > 0) and not F.Visible then
      begin
        if (Sender is TCustomSynEdit32) then
          DoExecute(Sender as TCustomSynEdit32)
        else
          if Assigned(Form.CurrentEditor) then
            DoExecute(Form.CurrentEditor as TCustomSynEdit32);
      end;
  end;
end;
...
begin
  ...
  begin
    if Pos(Key, TriggerChars) <> 0 then
      ActivateTimer(Sender as TCustomSynEdit32)
    else
      DeactivateTimer;
  end;
end;
  ...
  if i <> -1 then
    ...
    begin
      ...
      begin
        ...
        FPreviousToken := GetPreviousToken(Form.CurrentEditor as TCustomSynEdit32);
...
{                if Pos(AChar, FTriggerChars) > 0 then
            ...
            else DoExecute(Sender as TCustomSynEdit32);
            end;
          end;
        else DoExecute(Sender as TCustomSynEdit32);
        end;
...
begin
  ShortCutToKey (FShortCut,ShortCutKey,ShortCutShift);
  if not (Sender as TCustomSynEdit32).ReadOnly and
    (Shift = ShortCutShift) and (Key = ShortCutKey) then
  begin
    Execute(GetPreviousToken(Sender as TCustomSynEdit32), Sender as TCustomSynEdit32);
    FNoNextKey := True;
    Key := 0;
  end;
end;
...
var
  ...
  TmpOptions: TSynEdit32EditorOptions;
  OrigOptions: TSynEdit32EditorOptions;
