...
{$IFNDEF QWEBIDL}
unit SynEdit32.Highlighter.WebIDL;
{$ENDIF}
...
type
  TSynWebIDLSyn = class(TSynEdit32CustomHighlighter)
  private
    fRange: TRangeState;
    fTokenID: TtkTokenKind;
    fSymbolTokenID: TstkSymbolTokenKind;
    fIdentFuncTable: array [0..58] of TIdentFuncTableFunc;
    fArgumentsAttri: TSynEdit32HighlighterAttributes;
    fExtendedAttri: TSynEdit32HighlighterAttributes;
  ...
  published
    property ArgumentsAttri: TSynEdit32HighlighterAttributes read fArgumentsAttri write fArgumentsAttri;
    property CommentAttri: TSynEdit32HighlighterAttributes read fCommentAttri write fCommentAttri;
    property ExtendedAttri: TSynEdit32HighlighterAttributes read fExtendedAttri write fExtendedAttri;
    ...
    property TypesAttri: TSynEdit32HighlighterAttributes read fTypesAttri write fTypesAttri;
  end;
...
begin
  ...
  fArgumentsAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrArguments,
    SYNS_FriendlyAttrArguments);
  fArgumentsAttri.Style := [fsBold];
  fArgumentsAttri.Foreground := clNavy;
  AddAttribute(fArgumentsAttri);

  fCommentAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrComment,
    SYNS_FriendlyAttrComment);
  fCommentAttri.Style := [fsItalic];
  fCommentAttri.Foreground := clGreen;
  AddAttribute(fCommentAttri);

  fExtendedAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrExtended,
  ...
  fIdentifierAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrIdentifier,
    SYNS_FriendlyAttrIdentifier);
  AddAttribute(fIdentifierAttri);

  fKeyAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrReservedWord,
  ...
  fTypesAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrDataType, SYNS_FriendlyAttrDataType);
...
function TSynWebIDLSyn.GetDefaultAttribute(Index: Integer): TSynEdit32HighlighterAttributes;
begin
  case Index of
...
function TSynWebIDLSyn.GetTokenAttribute: TSynEdit32HighlighterAttributes;
begin
  case GetTokenID of
