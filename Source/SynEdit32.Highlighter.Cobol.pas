...
unit SynEdit32.Highlighter.Cobol;
...
type
  ...
  private
    ...
    FAIdentifierAttri: TSynEdit32HighlighterAttributes;
    ...
    FBooleanAttri: TSynEdit32HighlighterAttributes;
    FSpaceAttri: TSynEdit32HighlighterAttributes;
    FStringAttri: TSynEdit32HighlighterAttributes;
    FSequenceAttri: TSynEdit32HighlighterAttributes;
    FIndicatorAttri: TSynEdit32HighlighterAttributes;
    FTagAreaAttri: TSynEdit32HighlighterAttributes;
    FDebugLinesAttri: TSynEdit32HighlighterAttributes;
  ...
  published
    ...
    property AreaAIdentifierAttri: TSynEdit32HighlighterAttributes read FAIdentifierAttri write FAIdentifierAttri;
    ...
    property BooleanAttri: TSynEdit32HighlighterAttributes read FBooleanAttri write FBooleanAttri;
    ...
    property SequenceAttri: TSynEdit32HighlighterAttributes read FSequenceAttri write FSequenceAttri;
    property IndicatorAttri: TSynEdit32HighlighterAttributes read FIndicatorAttri write FIndicatorAttri;
    property TagAreaAttri: TSynEdit32HighlighterAttributes read FTagAreaAttri write FTagAreaAttri;
    property DebugLinesAttri: TSynEdit32HighlighterAttributes read FDebugLinesAttri write FDebugLinesAttri;
...
begin
  if IsLineEnd(FRun) then
    NextProcedure
  else if FRun < FCodeStartPos - 1 then
  begin
    FTokenID := tkSequence;
    repeat
      Inc(FRun);
    until (FRun = FCodeStartPos - 1) or IsLineEnd(FRun);
  end
  else
  begin
    FTokenID := tkIndicator;
    case FLine[FRun] of
      '*', '/', 'D', 'd': FIndicator := FLine[FRun];
...
begin
  ...
  while IsNumberChar and (FRun <= FCodeEndPos) do
  begin
    case FLine[FRun] of
      '.':
        if not CharInSet(FLine[FRun + 1], ['0'..'9', 'e', 'E']) then
          Break
        else
          fFloat := True;
      'e', 'E':
          if not CharInSet(FLine[FRun - 1], ['0'..'9', '.']) then
            Break
          else fFloat := True;
      '-', '+':
        begin
          if not fFloat or not CharInSet(FLine[FRun - 1], ['e', 'E']) then
            Break;
        end;
    end;
    Inc(FRun);
...
begin
  ...
  if FRun <= FCodeEndPos then
  repeat
    if (FLine[FRun] = StringChars[FRange])
      and ((FLine[FRun] <> '=') or ((FRun > 0) and (FLine[FRun - 1] = '='))) then
    begin
      if (FRun = FCodeEndPos) and (FRange in [rsQuoteString, rsApostString]) then
        Inc(FRange, 3)
      else
        FRange := rsUnknown;
      Inc(FRun);
...
begin
  ...
  begin
    ...
    if (FRange <> rsPseudoText) and (FRun <= FCodeEndPos) then
    repeat
      if (FLine[FRun] = StringChars[FRange]) then
      begin
...
begin
  ...
  FAIdentifierAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrAreaAIdentifier, SYNS_FriendlyAttrAreaAIdentifier);
  ...
  FBooleanAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrBoolean, SYNS_FriendlyAttrBoolean);
  ...
  FSequenceAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrSequence, SYNS_FriendlyAttrSequence);
  FSequenceAttri.Foreground := clDkGray;
  AddAttribute(FSequenceAttri);

  FIndicatorAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrIndicator, SYNS_FriendlyAttrIndicator);
  FIndicatorAttri.Foreground := clRed;
  AddAttribute(FIndicatorAttri);

  FTagAreaAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrTagArea, SYNS_FriendlyAttrTagArea);
  FTagAreaAttri.Foreground := clMaroon;
  AddAttribute(FTagAreaAttri);

  FDebugLinesAttri := TSynEdit32HighlighterAttributes.Create(SYNS_AttrDebugLines, SYNS_FriendlyAttrDebugLines);
...
begin
  ...
  begin
    ...
    if (FTokenID = tkIdentifier) and (FRun < FCodeMediumPos) then
      FTokenID := tkAIdentifier;
    Inc(FRun, FStringLen);

    while IsIdentChar(FLine[FRun]) and (FRun <= FCodeEndPos) do
      Inc(FRun);
  end;
end;
