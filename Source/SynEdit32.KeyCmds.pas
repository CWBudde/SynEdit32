...
The Original Code is: SynEdit32KeyCmds.pas, released 2000-04-07.
...
$Id: SynEdit32KeyCmds.pas,v 1.23.2.4 2008/09/14 16:24:58 maelh Exp $
...
type
  ...
  TSynEdit32KeyStroke = class(TCollectionItem)
  private
  ...
  TSynEdit32KeyStrokes = class(TCollection)
  private
    FOwner: TPersistent;
  ...
  public
    ...
    procedure SaveToStream(AStream: TStream);
  public
    property Items[Index: Integer]: TSynEdit32KeyStroke read GetItem
...
procedure TSynEdit32KeyStroke.Assign(Source: TPersistent);
begin
  if Source is TSynEdit32KeyStroke then
  begin
    Command := TSynEdit32KeyStroke(Source).Command;
    Key := TSynEdit32KeyStroke(Source).Key;
    Key2 := TSynEdit32KeyStroke(Source).Key2;
    Shift := TSynEdit32KeyStroke(Source).Shift;
    Shift2 := TSynEdit32KeyStroke(Source).Shift2;
  end else
...
procedure TSynEdit32KeyStroke.SetCommand(const Value: TSynEditorCommand);
begin
  if Value <> FCommand then
    FCommand := Value;
end;

procedure TSynEdit32KeyStroke.SetKey(const Value: Word);
begin
  if Value <> FKey then
    FKey := Value;
end;

procedure TSynEdit32KeyStroke.SetShift(const Value: TShiftState);
begin
  if Value <> FShift then
    FShift := Value;
end;

procedure TSynEdit32KeyStroke.SetShortCut(const Value: TShortCut);
var
  NewKey: Word;
...
begin
  ...
  begin
    ...
    Dup := TSynEdit32KeyStrokes(Collection).FindShortcut2(Value, ShortCut2);
...
procedure TSynEdit32KeyStroke.SetKey2(const Value: Word);
begin
  if Value <> FKey2 then
    FKey2 := Value;
end;

procedure TSynEdit32KeyStroke.SetShift2(const Value: TShiftState);
begin
  if Value <> FShift2 then
    FShift2 := Value;
end;

procedure TSynEdit32KeyStroke.SetShortCut2(const Value: TShortCut);
var
  NewKey: Word;
  NewShift: TShiftState;
  Dup: integer;
begin
  ...
  begin
    // Check for duplicate shortcut in the collection and disallow if there is.
    Dup := TSynEdit32KeyStrokes(Collection).FindShortcut2(ShortCut, Value);
    if (Dup <> -1) and (Dup <> Self.Index) then
      raise ESynKeyError.Create(SYNS_EDuplicateShortCut);
  end;
...
function TSynEdit32KeyStroke.GetShortCut2: TShortCut;
begin
...
{ TSynEdit32KeyStrokes }

function TSynEdit32KeyStrokes.Add: TSynEdit32KeyStroke;
begin
  Result := TSynEdit32KeyStroke(inherited Add);
end;

procedure TSynEdit32KeyStrokes.AddKey(const ACmd: TSynEditorCommand; const AKey: Word;
  const AShift: TShiftState);
var
  NewKeystroke: TSynEdit32KeyStroke;
begin
  NewKeystroke := Add;
  try
...
procedure TSynEdit32KeyStrokes.Assign(Source: TPersistent);
var
  x: integer;
begin
  if Source is TSynEdit32KeyStrokes then
  begin
    Clear;
    for x := 0 to TSynEdit32KeyStrokes(Source).Count-1 do
    begin
      with Add do
        Assign(TSynEdit32KeyStrokes(Source)[x]);
    end;
  end
  else
    inherited Assign(Source);
...
constructor TSynEdit32KeyStrokes.Create(AOwner: TPersistent);
begin
  inherited Create(TSynEdit32KeyStroke);
  FOwner := AOwner;
end;

function TSynEdit32KeyStrokes.FindCommand(Cmd: TSynEditorCommand): integer;
var
  x: integer;
begin
  Result := -1;
...
function TSynEdit32KeyStrokes.FindKeycode(Code: Word; SS: TShiftState): integer;
var
  x: integer;
begin
  Result := -1;
...
function TSynEdit32KeyStrokes.FindKeycode2(Code1: Word; SS1: TShiftState;
  Code2: Word; SS2: TShiftState): integer;
var
  x: integer;
begin
...
function TSynEdit32KeyStrokes.FindShortcut(SC: TShortcut): integer;
var
  x: integer;
begin
  Result := -1;
...
function TSynEdit32KeyStrokes.FindShortcut2(SC, SC2: TShortcut): integer;
var
  x: integer;
begin
  Result := -1;
...
function TSynEdit32KeyStrokes.GetItem(Index: Integer): TSynEdit32KeyStroke;
begin
...
procedure TSynEdit32KeyStrokes.ResetDefaults;
begin
  Clear;
...
procedure TSynEdit32KeyStrokes.SetItem(Index: Integer; Value: TSynEdit32KeyStroke);
begin
 inherited SetItem(Index, Value);
end;

procedure TSynEdit32KeyStrokes.SaveToStream(AStream: TStream);
var
  i, Num: integer;
begin
  Num := Count;
